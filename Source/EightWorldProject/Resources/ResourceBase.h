...
/**
 * 자원 베이스 클래스
 * 플레이어가 공격하여 자원을 채집할 수 있는 오브젝트의 기본 클래스
 */
UCLASS(Abstract)
class EIGHTWORLDPROJECT_API AResourceBase : public AActor
{
	GENERATED_BODY()
...
public:
	...
	// 작업 중 상태 설정
	UFUNCTION(BlueprintCallable, Category = "Resource")
	virtual void SetIsBeingWorkedOn(bool bInIsBeingWorkedOn);
...
protected:
	...
	// 리소스 비활성화 처리 가상함수 처리
	UFUNCTION()
	virtual void HandleDeactivation() PURE_VIRTUAL(AResourceBase::HandleDeactivation);
	
	// 리소스 메시 로드 가상함수 처리
	UFUNCTION()
	virtual void LoadResourceMesh() PURE_VIRTUAL(AResourceBase::LoadResourceMesh);
